...
// Fetch main country info (now that capital_name is removed from countries)
$stmt = $conn->prepare("
...
// Fetch all capitals from the separate table
...
// (Optional) fetch child entities (territories / de_facto_states)
...
<body>
    ...
    <div id="country-detail-card">
        ...
        <div class="card-content">
            ...
            <div class="country-info">
                <?php
                // Combine capitals for display
                if ($capitals) {
                    $capital_names = array_map(function($cap) {
                        $nameEsc = htmlspecialchars($cap['capital_name']);
                        $typeEsc = htmlspecialchars($cap['capital_type'] ?? '');
                        return $typeEsc ? "$nameEsc ($typeEsc)" : $nameEsc;
                    }, $capitals);
                ...
                <!-- If territory or de_facto_state => show parent -->
                <?php
                if (!empty($country['entity_type'])
                    ...
                    if ($parent) {
                        $parentName = htmlspecialchars($parent['country_name']);
                        ...
                        if ($country['entity_type'] === 'territory') {
                            echo "<p><strong>Administered by:</strong> 
                                  <a href='$parentLink'>$parentName</a></p>";
                        } else {
                    ...
                    <ul>
                        <?php foreach ($child_entities as $child): ?>
                            <li>
                                ...
                                <?php endif; ?>
                                <br><strong>Type:</strong> <?php echo htmlspecialchars($child['entity_type']); ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                <?php endif; ?>
    ...
    fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/<?php echo urlencode($country['country_name']); ?>.json?access_token=${mapboxgl.accessToken}&limit=1`)
        ...
        .then(data => {
            ...
            if (!usedGeocode) {
                console.warn('Mapbox geocoding failed; fallback to capital coords.');
            ...
            // Place markers for all capitals
            <?php foreach ($capitals as $cap) {
                if (!empty($cap['latitude']) && !empty($cap['longitude'])) {
                    $capJS = htmlspecialchars($cap['capital_name'], ENT_QUOTES);
            ?>
            new mapboxgl.Marker()
                .setLngLat([<?php echo $cap['longitude']; ?>, <?php echo $cap['latitude']; ?>])
                .setPopup(new mapboxgl.Popup().setHTML('<h3><?php echo $capJS; ?></h3>'))
                .addTo(map);
            <?php } } ?>
        })
        .catch(err => {
